...
3. **For each issue**, in priority order:
   ...
   - Merge into main and push:
     ...
     ```
   - If merge conflicts occur, resolve them, then commit the merge and push
   ...
   - Comment on the issue with this format:
     ...
     ## Branch
     `<branch-name>` — pushed and ready for review.
     ```
...
- Merge back into main with `--no-ff` and push main
...
- Merge into main and push — do not leave branches as open PRs
